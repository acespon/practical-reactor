...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Flux<Message> messageFlux = streamingService().flatMapMany(message -> message.share());
...
//        Flux<Integer> numbers = numberService1().concatWith(numberService2());
    ...
    public void task_executor_again() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor().concatMap(x -> x.then());
    ...
    public void plan_b() {
        //todo: feel free to change code as you need
        Flux<String> stonks = getStocksLocalCache().cache().switchIfEmpty(getStocksRest());
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((message, flux) -> {
            if (Objects.requireNonNull(message.get()).metaData.equals("spam")) {
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        Flux<String> committedTasksIds = tasksToExecute().concatMap(
                task -> task.flatMap(taskId -> commitTask(taskId)
                        .thenReturn(taskId))
        );
    ...
    public void car_factory() {
        //todo: feel free to change code as you need
        Flux<Car> producedCars = Flux.zip(carChassisProducer(), carEngineProducer()).concatMap(tuple -> {
                    Chassis chassis = tuple.getT1();
                    Engine engine = tuple.getT2();
                    return Mono.just(new Car(chassis, engine));
                }
        );
