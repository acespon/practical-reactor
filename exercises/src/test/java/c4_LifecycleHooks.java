...
public class c4_LifecycleHooks extends LifecycleHooksBase {
    ...
    public void no_subscription_no_gains() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service()
                .doOnSubscribe(next -> hooksTriggered.add("subscribe"))
    ...
    public void atomic_counter() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service()
                .doOnNext(element -> { // Add a behaviour (side-effect) when Flux emits an item
    ...
    public void one_to_catch_them_all() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service()
                .doFinally(x -> hooksTriggeredCounter.incrementAndGet()) // we needed tha
                //todo: change this line only
                ;
    ...
    public void one_to_rule_them_all() {
        ...
        Flux<Integer> flux = Flux.just(1, 2, 3)
                .doOnEach(x -> signals.add(x.getType().name())) //get the type of the signal, then retrieve the signal name
                //todo: change this line only
                ;
